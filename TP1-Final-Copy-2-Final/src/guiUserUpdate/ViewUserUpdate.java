...
public class ViewUserUpdate {
	...
	// Added these attributes to help with Password change
	public static javafx.scene.control.TextField text_NewPassword;
	public static javafx.scene.control.TextField text_ConfirmPassword;
	public static javafx.scene.control.Alert alertPasswordError;
	...
	private static boolean checkNewUser;	
	 ...
	 *
	 */
	public static void displayUserUpdate(Stage ps, User user, boolean newUser) {
		...
		// Will check to see if the user is new and will send them back to the login
		// screen if the are.
		if(checkNewUser == true) { 
			
			button_ProceedToUserHomePage.setText("Back to Login");
        	...
        	button_ProceedToUserHomePage.setText("Proceed to the User Home Page");
        	button_ProceedToUserHomePage.setOnAction((event) -> 
	...
	private ViewUserUpdate() {
        ...
        button_UpdatePassword.setOnAction((event) -> {
	        TextInputDialog newPw = new TextInputDialog("");
	        newPw.setTitle("Update Password");
	        newPw.setHeaderText("Enter a new password");
	        Optional<String> r1 = newPw.showAndWait();
	        if (!r1.isPresent()) return;
	
	        TextInputDialog confPw = new TextInputDialog("");
	        confPw.setTitle("Confirm Password");
	        confPw.setHeaderText("Confirm your new password");
	        Optional<String> r2 = confPw.showAndWait();
	        ...
	        if (p1.isEmpty()) {
	            new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.ERROR, "Password cannot be empty.").showAndWait();
	            return;
	        }
	        if (!p1.equals(p2)) {
	            new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.ERROR, "Passwords do not match.").showAndWait();
	            return;
	        }
	        ...
	        // Update in-memory User and label (mirrors how you update other fields)
	        theUser.setPassword(p1);
	        label_CurrentPassword.setText(p1);
	
	        new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.INFORMATION, "Password successfully updated.").showAndWait();
        });
        ...
        button_UpdateEmailAddress.setOnAction((event) -> {result = dialogUpdateEmailAddresss.showAndWait();
    		result.ifPresent(email -> {//name -> theDatabase.updateEmailAddress(theUser.getUserName(), result.get()));
    				if(!ControllerUserUpdate.invalidEmailAddress(email)) {
    			        ...
    			        invalidEmailAlert.setContentText("Example: user@example.com");
