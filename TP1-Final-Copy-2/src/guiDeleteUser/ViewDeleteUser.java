...
public class ViewDeleteUser {
	...
	*/
	...
	protected static String theSelectedUser = "<Select a User>";	// The user whose roles are being updated
	...
	*/
	...
	public ViewDeleteUser() {
		...
		// GUI Area 1
		label_PageTitle.setText("Delete Users Page");
		...
		for (String user : userList) {
			theDatabase.getUserAccountDetails(user);
			if (user.compareTo(theUser.getUserName()) != 0)
				filteredUsers.add(user);
		}
		
		combobox_SelectUser.setItems(FXCollections.observableArrayList(filteredUsers));
		...
		// GUI Area 2b
		setupButtonUI(button_DeleteUser, "Dialog", 18, 170, Pos.CENTER, 610, 125);
		ViewDeleteUser.button_DeleteUser.setOnAction((event) -> 
		...
		theRootPane.getChildren().addAll(label_PageTitle, label_SelectUser, label_UserDetails, combobox_SelectUser, button_UpdateThisUser, 
	...
	public static void refreshUsers() {
		...
		// Adds all users that aren't the logged in user
		List<String> filteredUsers = new ArrayList<String>();
		for (String user : userList) {
			theDatabase.getUserAccountDetails(user);
			if (user.compareTo(theUser.getUserName()) != 0)
