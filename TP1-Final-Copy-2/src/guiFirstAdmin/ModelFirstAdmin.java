...
public class ModelFirstAdmin {
	...
	public static boolean foundAlphabetic = false;            // Username conditions to check
	...
	protected static void checkUsername() {
		ViewFirstAdmin.resetUsernameDisplay();   //resets Username Display
		String username = ViewFirstAdmin.text_AdminUsername.getText(); // Fetches input from text input for username
		// If the input is empty, clear the display
		if (username.isEmpty()) {
			ViewFirstAdmin.resetUsernameDisplay();  // Reset the password requirements to be blank
		} else { 
	...
	public static String evaluateUsername(String input) {
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				// UserName is valid, update variables and display to match
	...
	// Update the code based on currently satisfied requirements
	private static void updateUNFlags() {
		ViewFirstAdmin.resetPasswordDisplay();
		ViewFirstAdmin.resetComparisonDisplay();
		ViewFirstAdmin.label_UNRequirements.setText("These are the Username requirements: ");
		ViewFirstAdmin.label_UNRequirements.setTextFill(Color.BLACK);
		if (foundAlphabetic) {
			ViewFirstAdmin.label_Alphabetic.setText("The First Character Must be a letter (A-Z or a-z) - Satisfied");
			ViewFirstAdmin.label_Alphabetic.setTextFill(Color.GREEN);
		} else {
			ViewFirstAdmin.label_Alphabetic.setText("The First Character Must be a letter (A-Z or a-z) - Not Satisified");
			ViewFirstAdmin.label_Alphabetic.setTextFill(Color.RED);
		}
		if (foundSPChar) {
			ViewFirstAdmin.label_SPChar.setText("The Username may contain a period, dash, or underscore (.,-,_)\n"
					+ "if it is followed by an alphanumerica character (A-Z, a-z, 0-9) - Not Satisified");
			ViewFirstAdmin.label_SPChar.setTextFill(Color.RED);
		} else {
			ViewFirstAdmin.label_SPChar.setText("The Username may contain a period, dash, or underscore (.,-,_)\n"
					+ "if it is followed by an alphanumerica character (A-Z, a-z, 0-9) - Satisified");
			ViewFirstAdmin.label_SPChar.setTextFill(Color.GREEN);
		}
		if (foundUNLongEnough) {
			ViewFirstAdmin.label_UNLongEnough.setText("At least four characters - Satisfied");
			ViewFirstAdmin.label_UNLongEnough.setTextFill(Color.GREEN);
		} else {
			ViewFirstAdmin.label_UNLongEnough.setText("At least four characters - Not Satisfied");
			ViewFirstAdmin.label_UNLongEnough.setTextFill(Color.RED);
		}
		if (foundUNTooLong) {
			ViewFirstAdmin.label_UNTooLong.setText("At most sixteen characters - Not Satisfied");
			ViewFirstAdmin.label_UNTooLong.setTextFill(Color.RED);
		} else {
			ViewFirstAdmin.label_UNTooLong.setText("At most sixteen characters - Satisfied");
			ViewFirstAdmin.label_UNTooLong.setTextFill(Color.GREEN);
		}
		if (validUsername) {
			ViewFirstAdmin.label_validUsername.setText("The Username is valid!");
			ViewFirstAdmin.label_validUsername.setTextFill(Color.GREEN);
			} else {
			ViewFirstAdmin.label_validUsername.setText("The Username is not valid!");
			ViewFirstAdmin.label_validUsername.setTextFill(Color.RED);
			}
	}
	...
	protected static void checkPassword() {
		ViewFirstAdmin.resetPasswordDisplay();   //resets Password Display
		String password = ViewFirstAdmin.text_AdminPassword1.getText(); // Fetches input from text input for password
		// If the input is empty, clear the display
		if (password.isEmpty()) {
			ViewFirstAdmin.resetPasswordDisplay();  // Reset the password requirements to be blank
		} else { 
			// If there is input, evaluate the passwords
			String errMessage = evaluatePassword(password);
			// Based on the current password, update flags
			updatePWFlags();
			...
			if (errMessage != "") { 
				...
				ViewFirstAdmin.errPasswordPart1.setText(password.substring(0, pwIndexofError));
				// The red arrow
				ViewFirstAdmin.errPasswordPart2.setText("\u21EB");
				// Tell the user about the meaning of the red up arrow
				ViewFirstAdmin.errPasswordPart3.setText("The red arrow points at the character causing the error!");
	...
	public static String evaluatePassword(String input) { 
		...
		// The following identify errors and improve the user's experience
		...
		//displayInputState();
			...
			// every time the first password is updated
			pwCurrentCharNdx++;
	...
	//  This method checks which conditions are met and updates the requirements shown
	private static void updatePWFlags() {
		ViewFirstAdmin.resetUsernameDisplay();
		ViewFirstAdmin.resetComparisonDisplay();
		ViewFirstAdmin.label_Requirements.setText("These are the pasword requirements: ");
		ViewFirstAdmin.label_Requirements.setTextFill(Color.BLACK);
		if (foundUpperCase) {
			...
			ViewFirstAdmin.label_UpperCase.setText("At least one upper case letter - Not Satisfied");
			ViewFirstAdmin.label_UpperCase.setTextFill(Color.RED);
		}
		if (foundLowerCase) {
			...
			ViewFirstAdmin.label_LowerCase.setText("At least one lower case letter - Not Satisfied");
			ViewFirstAdmin.label_LowerCase.setTextFill(Color.RED);
		}
		if (foundNumericDigit) {
			...
			ViewFirstAdmin.label_NumericDigit.setText("At least one numeric digit - Not Satisfied");
			ViewFirstAdmin.label_NumericDigit.setTextFill(Color.RED);
		}
		if (foundSpecialChar) {
			...
			ViewFirstAdmin.label_SpecialChar.setText("At least one special character - Not Satisfied");
			ViewFirstAdmin.label_SpecialChar.setTextFill(Color.RED);
		}
		if (foundLongEnough) {
			...
			ViewFirstAdmin.label_LongEnough.setText("At least eight characters - Not Satisfied");
			ViewFirstAdmin.label_LongEnough.setTextFill(Color.RED);
		}
		if (foundTooLong) {
			ViewFirstAdmin.label_TooLong.setText("At most thirty-two characters - Not Satisfied");
			ViewFirstAdmin.label_TooLong.setTextFill(Color.RED);
		} else {
			ViewFirstAdmin.label_TooLong.setText("At most thirty-two characters - Satisfied");
			ViewFirstAdmin.label_TooLong.setTextFill(Color.GREEN);
		}
	}
	...
	protected static boolean comparePasswords() {
		currentCharNdx = 0;
		// The passwords are pulled for comparison
		...
		// If either passwords is not provided, do not compare them
			if(password1.length() <= 0 || password2.length() <= 0)
				return false;
		// Clear the Password requirements to focus on if the codes match
		ViewFirstAdmin.resetPasswordDisplay();
		password1Char = password1.charAt(0);
		password2Char = password2.charAt(0);
		running = true;
		// If the passwords are not the same length, they are not the same
		...
		while(running) 
		{
			if (password1Char == password2Char) {
				// Go to the next character if there is one
				currentCharNdx++;
				// Stop the loop if checking an index past the first passwords length
				if (currentCharNdx >= password1.length())
					running = false;
				else {
					// Set the next characters for comparison
	...
	protected static void updateComparisonFlags() {
		// Clear the requirements space
		ViewFirstAdmin.resetUsernameDisplay();
		ViewFirstAdmin.resetComparisonDisplay();
		// Display whether the passwords match before attempting to create an admin account
		if(comparePasswords()) {
			ViewFirstAdmin.label_PasswordsDoNotMatch.setText("The passwords must match! - Satisified");
			ViewFirstAdmin.label_PasswordsDoNotMatch.setTextFill(Color.GREEN);
		} else {
			ViewFirstAdmin.label_PasswordsDoNotMatch.setText("The passwords must match! - Not Satisified");
